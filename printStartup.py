...
def print_prolog(parameters):
    ...
    parameter_string = '{:<' + str(max(len(mainConfig.configuration_parameters[key][0]) for key in parameters) + 1) + '} {}'
    ...
    for key, value in parameters.items():
        ...
        print(parameter_string.format(mainConfig.configuration_parameters[key][0] + ':', value))
...
def print_warnings(parameters):
    for key, value in parameters.items():
        ...
        if not value and type(value) == bool:
            print(f'Warning: The configuration parameter "{mainConfig.configuration_parameters[key][0]}" that you provided is invalid. Therefore the default was loaded ({str(mainConfig.configuration_parameters[key][2])})')
