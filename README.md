...
Among all the other data, [sondemod](http://dxlwiki.dl1nux.de/index.php?title=Sondemod) also provides RSSI values. And the upload of RSSI values is also [supported](https://github.com/projecthorus/sondehub-infra/wiki/SondeHub-Telemetry-Format) by [SondeHub Tracker](https://sondehub.org/). But since the values that [sondemod](http://dxlwiki.dl1nux.de/index.php?title=Sondemod) provides are coming from an uncalibrated [RTL-SDR](https://www.rtl-sdr.com/) stick, they do not represent anything that is remotely close to reality. Therefore, the upload of RSSI values is currently not supported.
