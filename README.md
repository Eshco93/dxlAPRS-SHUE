...
-s|Save the raw APRS packages to a textfile (0 = no / 1 = yes).<br />All packages in one file with one line for each package.|0|0 - 1
-w|Save the telemetry data to CSV files (0 = no / 1 = yes).<br />One CSV file for each radiosonde, named by it's serial with with `t_` as a prefix.|0|0 - 1
-z|Save the reformatted telemetry data to CSV files (0 = no / 1 = yes).<br />One CSV file for each radiosonde, named by it's serial with `r_` as a prefix.|0|0 - 1
